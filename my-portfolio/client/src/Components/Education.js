        ...
        <div className='flex flex-col border-l-1 border-tertiary gap-5 sm:flex-row sm:overflow-auto sm:scrollbar-hide sm:gap-7'>
          {education.map((exp, index) => (
            <div className='cursor-pointer'>
              <h1 key={exp.id} onClick={() => handleClick(index)} className={`${selectedItem === index ? 'text-tertiary border-tertiary -ml-1 bg-[#165232b5] p-2 xl:border-l-4' : 'text-white'}  px-5 sm:px-2 sm:w-[10vh] w-[20vh] text-xl sm:text-lg`}>{exp.degree}</h1>
            </div>
          ))}
        </div>
        <div className='flex sm:flex-col px-20 sm:px-2'>
          <div className='w-1/3 sm:w-full'>
            <img className={"border-2 rounded-lg sm:h-52 sm:w-72 h-60 w-72"} src={`${education[selectedItem].image}`} alt={education[selectedItem].title} />
          </div>
          <div className='text-white w-full px-10 sm:w-full sm:px-2 sm:py-2'>
            <div className=''>
              ...
              <p className='my-2'>Major in {education[selectedItem].major}</p>
